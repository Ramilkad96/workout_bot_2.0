...
def screen_day_comment(state: dict) -> tuple:
    ...
    text = (
        ...
        f"Как назвать этот день? Подойдёт что угодно:\n"
        f"• группа мышц — «Грудь + трицепс», «Ноги»;\n"
        f"• тип тренировки — «Фулбади», «Кардио», «Тяжёлая»;\n"
        f"• день недели — «Понедельник»;\n"
        f"• или просто «День {day['number']}».\n\n"
        f"Напишите название или нажмите «Пропустить» — "
        f"тогда день так и останется «День {day['number']}»." + CANCEL_HINT
    )
